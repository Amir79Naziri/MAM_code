...
data_config = {
    "path": "/local/home/am/EX_M/datasets/scRNA/retina",
    "seed": 32,
...
adata = sc.read(data_config['path'] + '/retina_preprocessed.h5ad')
...
to_split_df = pd.DataFrame({
    ...
    'age': adata.obs['age'],
    'sex': adata.obs['sex'],
    'donor_id': range(adata.shape[0])  # replace with adata.obs['donor_id'] if available
})
...
print(to_split_df[to_split_df['set'] == 'train']['age'].value_counts())
...
print(to_split_df[to_split_df['set'] == 'test']['age'].value_counts())
...
print(to_split_df[to_split_df['set'] == 'val']['age'].value_counts())
...
val_data.write(data_config['path'] + f'/seed_{random_state}/retina_val.h5ad')
...
transformed_val_data.to_csv(data_config['path'] + f'/seed_{random_state}/retina_val_with_expression{"_highly_variable" if data_config["only_highly_variable"] else ""}.csv', index=False)
...
test_data.write(data_config['path'] + f'/seed_{random_state}/retina_test.h5ad')
...
transformed_test_data.to_csv(data_config['path'] + f'/seed_{random_state}/retina_test_with_expression{"_highly_variable" if data_config["only_highly_variable"] else ""}.csv', index=False)
...
train_data.write(data_config['path'] + f'/seed_{random_state}/retina_train.h5ad')
...
transformed_train_data.to_csv(data_config['path'] + f'/seed_{random_state}/retina_train_with_expression{"_highly_variable" if data_config["only_highly_variable"] else ""}.csv', index=False)
